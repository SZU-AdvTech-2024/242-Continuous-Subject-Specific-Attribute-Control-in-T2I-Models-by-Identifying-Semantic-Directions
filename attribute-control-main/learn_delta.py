...
import sys
sys.argv.extend(["device=cuda:0", "model=sdxl", "prompts=people/age"])
    ...
    for global_step in (pbar := tqdm(range(max_steps), desc='Optimizing')):
        ...
        for accum_step in range(grad_accum_steps):
            with torch.no_grad():
                ...
                scale = (((torch.rand((scale_batch_size, batch_size), device=x_t.device) > .5).float() * 2 - 1) 
                         if randomize_scale_sign else False) * ((scale_max - scale_min) * torch.rand((scale_batch_size, batch_size), device=x_t.device) + scale_min)
                scale_cpu = scale.cpu()
            ...
            logger.info(f'        Accumstep {accum_step} loss:{loss_sum / (accum_step + 1)}')

        # pbar.set_description(f'Optimizing. Loss: {loss_sum / grad_accum_steps:.6f}')
        pbar.set_postfix({ 'loss': loss_sum / grad_accum_steps })
        logger.info(f'Globalstep {global_step} loss:{loss_sum / grad_accum_steps}')
        optimizer.step()
        optimizer.zero_grad()
